...
def get_twitter():
    ...
    try:
        ...
        twitter = Twython(
            settings.TWITTER_KEY, access_token=get_twitter_access_token(True)
        )
...
def get_twitter_last_statuses():
    ...
    if twitter_last_statuses is None:
        try:
            with open(
                settings.TEMPLATE_FILE_LOCATION + "twitter_laststatuses.json", "r"
            ) as f:
...
def check_twitter():
    ...
    for account in get_twitter_last_statuses().keys():
        user_timeline = twitter.get_user_timeline(
            screen_name=account, since_id=get_twitter_last_statuses()[account]
        )
        ...
        for tweet in user_timeline:
            ...
            if "no step free access" in tweet_text.lower():
                ...
                if station_name:
                    ...
                    problem["twitter-time"] = convert_tweet_time(
                        tweet["created_at"]
                    ).isoformat()
                ...
                "step free access" in tweet_text.lower()
                and "restored" in tweet_text.lower()
            ):
                station_name = find_station_name(h.unescape(tweet_text))

                problem = get_problem_for_station(global_problems, station_name)

                problem["twitter-resolved"] = convert_tweet_time(
                    tweet["created_at"]
                ).isoformat()
